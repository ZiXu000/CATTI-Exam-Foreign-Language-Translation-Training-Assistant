...
- **自定义模型**：支持配置并接入多种大语言模型（如 DeepSeek 等）作为智能批改引擎。

### 2. 🎧 口译全真模拟 (Interpretation Mode)
将任意一段中英文文本，瞬间转化为带有语音的口译实战考卷。
- **口译综合能力 (Comprehensive Ability)**：
  - 自动生成极具迷惑性的判断题、选择题（利用语音混淆、时空倒置等技巧）。
  - 自动提取摘要题的 5 大关键得分点。
  - **交互式判卷**：点击选项作答，交卷后直观展示红绿对错高亮反馈，并附带详尽解析。
